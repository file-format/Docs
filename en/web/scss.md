...
As the designs of websites are becoming complex resulting in large [CSS](/web/css/) files. Such files are harder to maintain. This is where SASS comes in. SASS introduces variables, nesting, mixins, imports, and selector inheritance in CSS development. These additions make working with design a lot more fun.
...
SASS files are not included directly in the [HTML](/web/html/) document like CSS. Instead, SASS files are converted to CSS files that are included in HTML files. To install SASS on your system, follow the instructions given on the [Official SASS Site](https://sass-lang.com/install).
To convert SASS to CSS, you may either convert an already saved SASS file or watch for changes and convert as the file is saved. The commands for both are given below.

### Convert Once ###

The first parameter is the source SASS file and the second parameter is the output CSS file.
...
The command has an additional *watch** flag that keeps the command running and as soon as the SASS file is saved, output CSS is generated.
