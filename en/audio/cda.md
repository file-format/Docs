...
## What is a CDA file?
A file with .cda extension is a small stub file generated by Microsoft Windows for each audio track on a an audio CD. These files contain the a typical information such as track times and a Windows shortcut that enables users to access the specific audio tracks. The CDA files are not music but they are pointing to a music file existing somewhere on the storage. We can say it as a short-cut of an audio file which is located on a CD.
