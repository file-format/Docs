...
DIB comes in the context of memory block as compared to .DIB fiels stored on disc. The memory block comprises of structure which is in accordance with the Windows API specifications for DIBs. Actual DIB consists of:
  * A header
  * Color palette
  * Pixel Data
...
|Bits|Description|
---|---|
|1-bit format (monochrome)|Monochrome bitmaps consist of two colors (black and white). Due to this limited number of colors, these bitmaps take less space on disc. The bitBitCount retuns true or false for representation of both the colors. Most of the applicaitons skip the palette completely if bitBitCount==1.
