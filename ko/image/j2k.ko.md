{
  "date" : "2019-10-11",
  "keywords" :[ "j2k 파일", "j2k 파일 형식", "j2k 파일이란", "파일", "j2k 예제", "j2k 파일 확장자","확장자", "형식" ],
  "author" : {
  ...
  "toc" : true,
  "title" :"J2K",
  "description":"J2K 파일을 만들고 열 수 있는 J2K 파일 형식과 API에 대해 알아보세요.",
  "linktitle" : "J2K",
  "menu" : {
    "docs" : {
      "parent" : "image"
...
**J2K** 파일은 DCT 압축 대신 웨이블릿 압축을 사용하여 압축한 이미지입니다. 이 파일 형식은 JPEG(Joint Photographic Experts Group) 2000 파일에서 사용됩니다. J2K 파일은 이를 위해 EXIF 형식을 사용하는 .jpeg 또는 .jpg와 달리 이미지 파일에 대한 메타데이터 정보를 XML로 저장합니다. J2K 파일은 15비트 색상, 알파 투명도 및 무손실 압축을 지원합니다. J2K-Codec과 같은 JPEG 2000 이미지를 디코딩하기 위한 여러 상용 API가 있습니다. J2K 파일은 표준 이미지 뷰어를 사용하여 Windows OS에서 열 수 있습니다.

## J2K 파일 형식 ##

J2K 파일 형식은 .jp2 및 .jpc로 저장되는 경우가 많은 JPEG 2000과 동일합니다. 따라서 J2K 파일은 표준 12234-1이 Exif 태그와 XML 구성 요소 간의 참조로 사용되는 XML 형식의 메타데이터 인코딩과 동일한 접근 방식을 따릅니다. 애니메이션 메커니즘과 코드 스트림 구성을 하나의 단일 이미지로 결합하는 JPEG 2000 part-2 확장으로 더욱 향상되었습니다. 이러한 확장 파일 형식 파일은 .jpx로 저장됩니다.
...
J2K는 빠른 압축 해제를 활용하고 픽셀 데이터에 필요한 정보만 가져와서 가시적 이미지의 일부를 화면에 빠르게 렌더링합니다. J2K는 주로 데이터를 보고 편집하지 않도록 설계되었습니다.

## J2K 식별 ##
JPEG 2000 파일의 서명 바이트는 6A 50 20 20입니다.
