{
  ...
  "draft" : "false",
  "toc" : true,
  "title" :"Fichier DER - Format de fichier de certificat DER",
  "description":"En savoir plus sur le format de fichier DER et les API qui peuvent créer et ouvrir des fichiers DER.",
  "linktitle" : "DER",
  "menu" : {
    "docs" : {
      "identifier":"web-der",
...
Un fichier DER est un fichier de certificat numérique créé et stocké au format binaire. Il s'agit d'un encodage binaire pour les certificats X.509 et les clés privées. Contrairement aux fichiers PEM ([CRT](https://docs.fileformat.com/web/crt/), [CER](https://docs.fileformat.com/web/cer/)), les fichiers DER ne pas contenir d'instructions en texte brut lisibles par l'homme telles que -----BEGIN CERTIFICATE-----. Ils sont principalement utilisés dans le contexte de Java.

## Format de fichier DER - Plus d'informations

Les fichiers DER sont stockés sur disque dans un format de fichier binaire qui n'est pas lisible par l'homme. Ces fichiers de certificat sont chargés et reconnus par les navigateurs Web standard pour une navigation sécurisée.

### À quoi ressemble un certificat encodé DER ?

Un exemple de certificat SSL/TLS codé DER est illustré ci-dessous.
...
## Convertir un certificat encodé DER en PEM

Un fichier de certificat codé DER peut être converti en PEM à l'aide de la commande suivante.
