...
Файлы Java HS имеют формат файла XML и основаны на предложенной рекомендации Расширенного языка разметки World Wide Web Consortium (W3C) [PR-xml-971208](http://www.w3.org/TR/PR-xml- 971208). Это означает, что файл Java HS имеет удобочитаемый формат файла XML, который можно открыть в любом приложении для чтения XML.
...
Код можно записать в файл .hs и скомпилировать с помощью компилятора Haskell, такого как [GHC](http://haskell.org/ghc). Следующая строка кода сохраняется как HelloWorld.hs, как показано в следующем примере.
