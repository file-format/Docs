{
  "date" : "2019-10-11",
  "keywords" :[ "vsx ファイル", "vsx ファイル形式", "vsx ファイルとは", "ファイル", "vsx の例", "vsx ファイルの拡張子","拡張子", "形式" ],
  "author" : {
    "display_name" : "Kashif Iqbal"
},
  "draft" : "false",
  "toc" : true,
  "title" :"VSX - Microsoft Visio ステンシル ファイル形式",
  "description":"VSX ファイル形式と,VSX ファイルを作成して開くことができる API について学びます。",
  "linktitle" : "VSX",
  "menu" : {
    "docs" : {
...
拡張子が .vsx のファイルは、Microsoft Visio で図を作成するために使用される図面と図形で構成されるステンシルを指します。 VSX ファイルは XML ファイル形式で保存され、Visio 2013 までサポートされていました。これらは、Microsoft Visio 2013 で導入された主要な [VSDX](/visio/vsdx/) ファイル形式とは異なります。VSX ファイルは、任意のテキスト エディターで開くことができます。コンテンツを表示します。 VSX ファイルは、PDF や HTML などのさまざまなファイル形式に変換できます。

## VSX ファイル形式 ##

XML エディタで開いた VSX ファイルの例を以下に示します。一般的な VSX ファイルには、その XML 表現からわかるように、次のノードがあります。

### ドキュメントのプロパティ ###

ドキュメント プロパティ セクションに含まれる要素の一部は次のとおりです。
...
一般的なドキュメント設定ノードには、次の要素が含まれています。
...
色ノードには、ドキュメント全体で使用される色のエントリが含まれています。
...
ファイル内の FaceNames ノードは、ドキュメントで使用される TypeFace.FaceNames プロパティを表します。
...
StyleSheets ノードには、ドキュメントで使用されているスタイル シートに関する情報が含まれています。各スタイル シートには、スタイル プロパティ、線、塗りつぶしパターン、テキスト ブロック、およびその他の要素に関する情報を含めることができます。

### 書類シート ###

DocumentSheet ノードには、ドキュメント プロパティとユーザー名情報に関する情報が含まれており、次のように表示されます。
...
ページ プロパティ、レイアウト、ルーラー グリッド、図形、画像、その他の同様のデータなど、Visio ドキュメントのページに関する情報が含まれています。
