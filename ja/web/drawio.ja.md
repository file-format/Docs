{
  "date" : "2019-10-11",
  "keywords" :[ "drawio","drawio ファイル", "drawio ファイル形式", "drawio ファイルの種類", "ファイル", "種類", "drawio ファイルとは" ],
  "author" : {
    "display_name" : "Kashif Iqbal"
},
  "draft" : "false",
  "toc" : true,
  "title" :"DRAWIO - Diagram.net ダイアグラム ファイル形式",
  "description" :"DRAWIO ファイル形式と,DRAWIO ファイルを作成して開くための API について学びます。",
  "linktitle" : "DRAWIO",
  "menu" : {
    "docs" : {
      "parent" : "web"
...
拡張子が .drawio のファイルは、ダイアグラムを操作するためのオープンソース プログラムである [diagrams.net](https://www.diagrams.net/) の draw.io で作成された描画ファイルです。テキスト、画像、レイアウト、形状、配置など、ダイアグラム要素のコンテンツとフォーマットに関する全体的な情報が含まれています。 DRAWIO がサポートする図には、フローチャート、組織図、地図、エンジニアリング要素、プロセス図、チャートなどが含まれます。 DRAWIO ファイルは、[JPG](/image/jpeg/)、[PNG](/image/png/)、[BMP](/image/bmp/)、[XML](/web/xml/) など、いくつかの異なる形式にエクスポートできます。、PDF、[HTML](/web/html/)、[VSDX](/visio/vsdx/)。

## DRAWIO ファイル形式

DRAWIO ファイルはベクター画像ファイルで、標準の XML ファイル形式で保存されます。 diagrams.net によって開発され、Microsoft Visio と同様のダイアグラム情報を保存する機能を提供します。 DrawIO は [オンライン アプリ](https://app.diagrams.net/) として利用でき、図を作成して開き、さまざまな形式にエクスポートできます。このアプリは、Chrome、Firefox、Edge、Safari などの主要なブラウザーで実行されるインタラクティブなグラフおよびチャート アプリケーションを提供する mxGraph ダイアグラム ライブラリに基づいています。

### DRAWIO の例

次の例は、DRAWIO アプリで作成した簡単なフローチャートです。

{{< figure src="../DRAWIO.png" alt="DRAWIO ファイル形式" height="421" width="291">}}

エクスポートで生成される出力 XML は次のとおりです。
...
* [DRAWIO - オンラインアプリ](https://app.diagrams.net/)
