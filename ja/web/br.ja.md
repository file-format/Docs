...
BR ファイルは、オープン ソースのデータ圧縮アルゴリズム **Brotli** を適用して生成された圧縮 Web ファイルです。スタイルシート ([CSS](/web/css/))、画像 ([SVG](/image/svg/))、[XML](/web/xml/)、スクリプトなどの Web ページ アセットを格納するために使用されます。ファイル ([JS](/web/js/))。 Chrome や Firefox などの現代の Web サイトでは、BR ファイルを使用してページの読み込み時間を短縮し、ユーザー エクスペリエンスを向上させています。
