{
  "date" : "2021-06-17",
  "keywords" :[ "DBC", "拡張子", "dbc ファイル", "dbc ファイル形式", "データベース ファイルの種類", "データベース ファイル形式", "dbc ファイルとは" ],
  "author" : {
    "display_name" : "Muhammad Umar"
},
  "draft" : "false",
  "toc" : true,
  "description" :"DBC ファイル形式と,DBC ファイルを作成して開くことができる API について学びます。",
  "title" :"DBC - CANデータベースファイル",
  "linktitle" : "DBC",
  "menu" : {
    "docs" : {
      "parent" : "database"
...
## .DBC オプション番号
拡張子が .dbc のファイルは、CAN データベース ファイルとも呼ばれます。 DBC ファイルは、生の CAN バス データを物理的な値または人間が読める形式にデコードするための情報で構成される単純なテキスト ファイルです。 DBC ファイルが導入されました。これは、データの識別と変換を管理するための非常に一般的な方法であるためです。 DBC ファイル タイプは、CAN ネットワークで記述されているように記録を保持する手段を提供するために開発されました。

## DBC ファイル形式
DBC ファイル形式は読み取り部分または受動部分のみを表しており、送信を詳しく説明する手段は提供していません。特定のネイティブ適応がまだない車両をサポートするため。これは、一般的な DBC 車両タイプを使用して行われ、DBC ファイルを使用して CAN データをメトリックに変換します。 DBC 内の各メッセージは、シグナルが C 構造体のメンバーである C 構造体になります。

### DBC 構造
DBC データは、次の要素で構成されます。

#### シンプルな DBC メッセージ
単純な DBC メッセージは、メッセージ ID と少なくとも 1 つのシグナルで構成されます。以下は、単一の 8 ビット信号で構成されるメッセージのデモです。
...
```
#### 署名付きシグナル
符号付き信号は、信号に負のオフセットを適用することで送信できます。以下は、前のメッセージへの署名付きシグナルの例です。
...
```
#### 列挙型
列挙型は、ユーザーが数字ではなく名前を表示したい場合に使用されます。次の例を参照してください。
...
```
#### 多重化されたメッセージ
単一のメッセージ ID を使用する多重化されたメッセージを定義することはオプションですが、送信された多重化された値に応じて異なる方法でデコードされます。以下の多重化されたメッセージを送信するには、別のメッセージを送信する必要があります。
