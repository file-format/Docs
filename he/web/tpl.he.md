{
  "date" : "2019-10-11",
  "keywords" :[ "tpl", "קובץ tpl", "פורמט קובץ tpl", "סוג קובץ tpl", "קובץ", "סוג", "מהו קובץ tpl" ],
  "author" : {
    "display_name" : "Kashif Iqbal"
},
  "draft" : "false",
  "toc" : true,
  "title" :"TPL - תבנית שרת קבצים של HTTP",
  "description" :"למד על מהו קובץ TPL וממשקי API ליצירה ופתיחה של קבצי TPL.",
  "linktitle" : "TPL",
  "menu" : {
    "docs" : {
      "parent" : "web"
...
קובץ עם סיומת .tpl הוא קובץ תבנית שנוצר ומשמש את HTTP File Server (HFS) שהוא תוכנת יישום לשיתוף קבצים לשליחה וקבלה של קבצים בטכנולוגיית האינטרנט HTTP במקום פרוטוקול FTP. הוא משמש לבניית דפי [HTML](/he/web/html/) באופן דינמי על סמך מידע התבנית שמוגדר כבעל הגדרות עם פריסה, סגנון ותסריטים דומים. לאפליקציות הדורשות אותן הגדרות ניתן להקצות את אותו קובץ תבנית ו-HFS יחזיר את הדף המבוקש באופן דינמי בזמן ריצה בהתבסס על קובץ תבנית זה.


## פורמט קובץ TPL

קבצי TPL מאוחסנים בפורמט קריא אנושי ומכילים HTML שהיא שפת סימון קריא אנושית. בנוסף ל-HTML, קובץ TPL עשוי להכיל גם CSS ו-Javascript ברמת התבנית כדי להגדיר את הפריסה והפעולות שיבוצעו על ידי כל הדפים שנוצרו מקובץ תבנית זה.

### קטעי TPL

קטעי TPL מכילים את קוד HTML, CSS או JavaScript המשמש את HFS בעת יצירת דף הפלט. כל קטע מתחיל בסוגריים מרובעים ([]) ומוגדר על ידי סימן אחוז (%). קובץ TPL עשוי לכלול את הסעיפים הבאים.

#### מדור סגנון TPL

הוא כולל את המידע הקשור לסגנון שעשוי להשתמש בהפניה לקובצי CSS משובצים או לא. דוגמה למדור סטיילינג היא כדלקמן.
...
קטע הקישור מכיל מידע על כתובת האתר ויכול לכלול הפניות לאירועים כגון כפתור והפעולה שלו.
...
מדור ההערות של TPL כולל מיועד ליצירת הערות ומוגדר כדלקמן.
```
[comment]
...
קטע ההעלאה מחזיר את תגובת ה-HTML בפועל מהשרת בהתבסס על הגדרות התבנית כפי שמוצג בדוגמה הבאה.
...
* [דוגמה TPL - Github](https://github.com/heiswayi/hfs-templates/blob/master/HFSTemplate_myHFS.tpl)
