{
  "date": "2019-10-11",
  "author": {
    "display_name": "Kashif Iqbal"
},
  "draft": "false",
  "toc": true,
  "title": "Formáid Chomhaid PDF/X",
  "description": "Foghlaim faoi fhormáid comhaid PDF/X agus APIanna ar féidir comhaid PDF/X a chruthú agus a oscailt.",
  "linktitle": "PDF/X",
  "menu": {
    "docs": {
      "parent": "pdf",
      "identifier": "pdf--gax"
}
},
  "lastmod": "2019-09-10"
}

# Cad is comhad PDF/X ann? #

Is caighdeán ISO 15930 é PDF/X a foilsíodh in 2001 le fothacar d’fheidhmiúlacht PDF. Bunaíodh agus foilsíodh an caighdeán bunaithe ar riachtanais shonracha na dtionscal priontála agus foilsitheoireachta. Ceapadh na ceanglais don chaighdeán seo go léir de réir riachtanais éagsúla na dtionscal priontála agus foilsitheoireachta. Éilíonn PDF/X go bhfuil na comhaid chomhlíonta iomlána ie féinchuimsitheach. Éilíonn sé seo go mbeadh gnéithe cosúil le clónna a úsáidtear ar an leathanach mar chuid den doiciméad. Ní féidir le hábhar ar nós 3D nó físeáin a bheith mar chuid de dhoiciméad PDF/X. Éilíonn an fhaisnéis atá i ndoiciméad PDF/X go bhfuil sé cruinn.

## PDF/X Caighdeáin agus Athbhreithnithe ##

Cuimsíonn an teaghlach caighdeán PDF/X roinnt leaganacha, gach ceann acu deartha le haghaidh toradh speisialaithe. Bhí sé mar aidhm le forbairt na gcaighdeán seo aghaidh a thabhairt ar na riachtanais éagsúla atá ag na tionscail priontála agus foilsitheoireachta.

* `PDF/X-1a` - Ar a dtugtar an chéad chaighdeán de theaghlach PDF/X freisin, éilíonn PDF/X-1a go mbeadh gach ábhar mar chuid den doiciméad PDF ionas gur féidir é a phriontáil. Ní cheadaítear eilimintí doiciméad ar nós clónna, foirmeacha, cosaint pasfhocail, agus nótaí sofheicthe. Tá a cheanglais shonracha féin ag PDF/X-1a chomh maith, amhail na ceanglais a bhaineann le trédhearcacht agus ceadaítear sraitheanna. Éilíonn siad seo freisin nach n-úsáidfidh eilimintí priontála ach CMYK, liathscála nó spotdathanna, rud a fhágann nach n-úsáidfear spásanna datha RGB nó gléas-neamhspleách. is le haghaidh malartuithe ina bhfuil gach comhad le seachadadh i CMYK (agus/nó spotdathanna), gan aon RGB nó sonraí arna mbainistiú ag dathanna. Tagraítear do PDF/X-1a freisin mar Malartú Iomlán mar gheall ar chomhláine an eolais a theastaíonn ó

* `PDF/X-3` - tugadh isteach é i 2002 agus baineadh go leor srianta PDF/X-1a. Chuir sé seo ar chumas na ngrafaicí i PDF/X-3 spásanna datha CMYK, grescale, RGB, Lab agus ICC a úsáid. Tá sé bunaithe i ndáiríre ar chaighdeáin PDF 1.3 le [próifíl ICC](https://en.wikipedia.org/wiki/ICC_Profile). Tugtar Bainistíocht Dathanna air freisin mar gheall ar an tsolúbthacht agus na rialacha a thugann sé isteach maidir le dathanna atá san áireamh i ndoiciméad PDF.

* `PDF/X-4` - tacaíonn sé le trédhearcacht, agus mar sin tá na sonraí go léir a theastaíonn le haghaidh aschur gan leata i PDF-X/4.

* Tá `PDF/X-5` - bunaithe ar PDF/X-4, ag cur tacaíocht le haghaidh grafaicí seachtracha trí XObjects tagartha, chomh maith le próifílí seachtracha n-dathanna chun rún rindreála. Úsáid é chun sonraí priontála a mhalartú go páirteach ag baint úsáide as leagan PDF 1.6


## Tagairtí ##

* [PDF/X - Le Vicipéid](https://en.wikipedia.org/wiki/PDF/X)


