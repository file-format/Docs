...
带有 .aux 扩展名的文件是一个辅助文件，其中包含 [.JPG](/zh/image/jpg/) 或 GeoTIFF [.TIFF](/zh/image/tiff/) 文件中伴随的栅格数据文件的地理空间数据。由于光栅文件不能保存此类信息，因此 AUX 文件用于以关联数据的形式存储附加信息。这些通常存储在与相应光栅文件名相同的文件名的同一目录中。 AUX 文件通常由 ESRI ArcGIS Desktop 和 ERDAS IMAGINE 等地理空间应用程序使用。
