...
GSM 文件使用有损 CBR（恒定比特率）声音压缩编解码器进行编码。 GSM 音频文件中的采样率为 8000 个样本/秒，而数据速率约为 13 kbps。 GSM 文件中的比特率可确保录音过程中的质量。此外，GSM 文件格式也被称为手机中使用的全球标准格式，WAV 文件也可以使用 GSM 文件格式轻松编码。 GSM 文件的任何问题都可以由用户自己轻松解决，而无需求助专家。用户还可以将 GSM 文件转换为 [MP3](/zh/audio/mp3/) 和 [MP4](/zh/audio/mp4/) 格式。
