...
## 什么是 .aiff 文件？
AIFF（音频交换文件格式）是 Apple 于 1998 年开发的一种未压缩音频文件格式，但基于 **EA IFF 85**（Electronic Arts 开发的交换格式文件标准），一种用于 Amiga 系统的包装格式.这种文件格式提供了存储采样声音的标准。该格式具有足够的灵活性，并且能够以各种采样率和采样宽度存储单声道或多声道采样声音。由于 AIFF 文件是未压缩的，这使得它们的大小比其他有损格式（例如 [MP3]（/audio/mp3/））更大。这些文件由 2 个未压缩立体声音频通道组成，样本大小为 16 位，以 44.1 khz 录制。由于音频质量高，5 分钟的音频最多可占用 50MB 磁盘空间，类似于 [WAV](/audio/wav/) 文件格式。

## AIFF 与 WAV
AIFF 和 WAV 在质量方面几乎相同。它们都使用相同的 PCM（脉冲编码调制）编码，这使得它们与其他有损格式相比更大，例如 [MP3](/audio/mp3/)， [M4A](/audio/m4a/)。下表列出了一些一般差异：
