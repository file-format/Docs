{
  "date" : "2021-06-17",
  "keywords" :["DBC","扩展名","dbc 文件","dbc 文件格式","数据库文件类型","数据库文件格式","什么是 dbc 文件"],
  "author" : {
    "display_name" : "Muhammad Umar"
},
  "draft" : "false",
  "toc" : true,
  "description" :"了解 DBC 文件格式和可以创建和打开 DBC 文件的 API。",
  "title" :"DBC - CAN 数据库文件",
  "linktitle" : "DBC",
  "menu" : {
    "docs" : {
...
## 什么是一 .dbc 文件？
扩展名为 .dbc 的文件也称为 CAN 数据库文件。 DBC 文件是一个简单的文本文件，包含用于将原始 CAN 总线数据解码为物理值或人类可读形式的信息。引入 DBC 文件，因为这是管理数据识别和转换的非常常用的方法。 DBC 文件类型的开发是为了提供如 CAN 网络中所述的记录保存方式。

## DBC 文件格式
DBC 文件格式仅代表读取或被动部分，它不提供详细传输的方法。支持还没有特定原生适配的车辆。这是使用通用 DBC 车辆类型使用 DBC 文件将 CAN 数据转换为指标来完成的。 DBC 中的每条消息都成为一个 C 结构，其中信号是 C 结构的成员。

### DBC结构
DBC 数据由以下元素组成：

#### 简单的 DBC 消息
一条简单的 DBC 消息由消息 ID 和至少一个信号组成。以下是由单个 8 位信号组成的消息的演示。
...
```
#### 有符号信号
可以通过对信号应用负偏移来发送带符号的信号。以下是前一条消息的签名信号示例。
...
```
#### 枚举类型
当用户想要查看名称而不是数字时，使用枚举类型。请参阅以下示例。
...
```
#### 多路复用消息
定义使用单个消息 ID 的多路复用消息是一种选择，但是，根据发送的多路复用值，它们的解码方式不同。为了在下面发送多路复用消息，必须发送单独的消息：
...
* [DBC 格式](http://socialledge.com/sjsu/index.php/DBC_Format)
* [J1939 和 DBC 文件简介](https://www.kvaser.com/developer-blog/an-introduction-j1939-and-dbc-files/)
